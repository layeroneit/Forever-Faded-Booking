...
|-----------|-----------------------------|--------|
| `MAIL_FROM` | `noreply@layeroneconsultants.com` | Must be a **verified** SES identity (domain or email). |
...
2. Use that identity as `MAIL_FROM` (e.g. `noreply@layeroneconsultants.com`).
...
3. **MAIL_FROM** — In Amplify env, set `MAIL_FROM` to the exact verified identity (e.g. `noreply@layeroneconsultants.com`). Redeploy/restart sandbox after changing env.
